...
const Logo = memo(({ onClick }: { onClick: () => void }) => (
  ...
  >
    ...
    <span className="text-md md:text-2xl font-bold md:font-bold">
      Weatherly
    </span>
  </div>
));
...
const NavBar = memo(() => {
  ...
  // Function to handle city selection and trigger weather data fetch
  const handleCitySelectInternal = (city: {
    name: string;
    country: string;
    lat: number;
    lon: number;
    ...
    handleCitySelect(city); // Ensure this function correctly fetches weather data

    // Fetch weather data for the selected city
    weatherAPI
      .fetchWeatherData(city.lat, city.lon)
      .then((data) => {
        console.log("Weather Data:", data); // Handle weather data (e.g., set it in state or display it)
      })
      .catch((err) => {
                ...
                {isOpen && (
                  <div className="relative">
                    <SearchSuggestions
                      ...
                      onSelect={handleCitySelectInternal}
...
//     // Optionally, save current location to localStorage if you have access to it
//     // For example, if you have lat/lon from the browser's geolocation API
